...
def _extract_docx_images_as_base64(docx_path: str) -> list[str]:
    """
    Extract every embedded image from a .docx file (ZIP archive) and
    return them as a list of base64-encoded JPEG strings.
    """
    images_b64 = []
...
def _inject_docx_images(elements: list, docx_path: str) -> list:
    """
    Append synthetic Image elements for every picture found in the .docx
    so the rest of the pipeline handles them exactly like PDF images.
    """
    from unstructured.documents.elements import Image as UnstructuredImage
 
    images_b64 = _extract_docx_images_as_base64(docx_path)
    if not images_b64:
        return elements
 
    for b64 in images_b64:
        img_element = UnstructuredImage(text="")
        img_element.metadata.image_base64 = b64
        img_element.metadata.image_mime_type = "image/jpeg"
...
def partition_document(temp_file: str, file_type: str, source_type: str = "file"):
    ...
    dispatch = {
        ...
        "docx": lambda: _inject_docx_images(
                    partition_docx(
                        filename=temp_file,
                        strategy="hi_res",
                        ...
                        # Note: extract_image_* params are NOT supported by partition_docx
                        # Images are handled separately by _inject_docx_images above
                    ),
                    temp_file,
                ),
        "pptx": lambda: partition_pdf(
            filename=convert_pptx_to_pdf(temp_file),
            strategy="hi_res",
            infer_table_structure=True,
            extract_image_block_types=["Image"],
            extract_image_block_to_payload=True,
        ),  
        "txt": lambda: partition_md(filename=temp_file),
